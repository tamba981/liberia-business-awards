// Run this to fix and verify your spotlight data
...
const newsCategorySchema = new mongoose.Schema({
    ...
    icon: String, color: String, display_order: Number, is_active: Boolean
});

const newsArticleSchema = new mongoose.Schema({
    category_id: { type: mongoose.Schema.Types.ObjectId, ref: 'NewsCategory' },
    title: String, slug: String, excerpt: String, content: String,
    author_name: String, business_name: String, business_owner: String,
    featured_image: String, status: String, published_at: Date,
    view_count: Number, is_featured: Boolean, is_breaking: Boolean, is_interview: Boolean
});
...
async function fixSpotlight() {
    try {
        ...
        const categories = await NewsCategory.insertMany([
            {
                name: 'Startup Spotlight', slug: 'startup-spotlight',
                description: 'Featured stories of emerging Liberian startups',
                icon: 'fa-rocket', color: '#FF0000', display_order: 1, is_active: true
            },
            {
                name: 'SME Success', slug: 'sme-success',
                description: 'Small and medium enterprises making an impact',
                icon: 'fa-building', color: '#87CEEB', display_order: 2, is_active: true
            },
            {
                name: 'Women Entrepreneurs', slug: 'women-entrepreneurs',
                description: 'Celebrating Liberian women in business',
                icon: 'fa-female', color: '#F59E0B', display_order: 3, is_active: true
            },
            {
                name: 'Business Interviews', slug: 'business-interviews',
                description: 'In-depth conversations with business leaders',
                icon: 'fa-microphone', color: '#8B5CF6', display_order: 4, is_active: true
            }
        ]);
        console.log('✅ Created', categories.length, 'categories');

        // Create articles with proper image URLs
        const now = new Date();
        ...
        // Verify counts
        const catCount = await NewsCategory.countDocuments();
        const artCount = await NewsArticle.countDocuments();
